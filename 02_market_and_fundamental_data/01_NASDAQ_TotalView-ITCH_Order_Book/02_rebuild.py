...
data_path = Path('data')  # set to e.g. external hard drive
...
def save_orders(orders, append=False):
    ...
    for buysell, book in orders.items():
        df = (pd.concat([pd.DataFrame(data=data, columns=cols).assign(timestamp=t) for t, data in book.items()]))
...
sns.despine()
fig.tight_layout()
plt.savefig('data/order_dist.png')
...
sns.despine()
fig.tight_layout()
plt.savefig('data/order_depth.png')
